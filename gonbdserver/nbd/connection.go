	...
	for {
		...
		case req, ok := <-c.rxCh:
			...
			case NBD_CMD_WRITE, NBD_CMD_WRITE_ZEROES:
				if req.nbdReq.NbdCommandType == NBD_CMD_WRITE_ZEROES {
					fmt.Println("NBD_CMD_WRITE_ZEROES:", int64(addr), length)
				} else {
					fmt.Println("NBD_CMD_WRITE:", int64(addr), length)
				}
				for i := 0; length > 0; i++ {
					blocklen := c.export.memoryBlockSize
					if blocklen > length {
